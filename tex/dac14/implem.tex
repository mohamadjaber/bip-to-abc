...
\cci{do-together} construct with the corresponding openmp directives. 
...
\todo[inline]{Not sure if we should mention how the resulting AIG resulting can be 
  transformed directly into a bit-level \caig, but then needs to use some sort 
  of technology mapping to take it into a word-level \caig that becomes an
optimized parallel implementation of the run-time verification...}
