...
The next state function refers to the wire variable $timer.timer.e$
as an input to one of its AND gates. 
The circuit for $timer.timer.e$ is also constructed by traversing 
the right hand side expression of its 
assignment statement in the wire definition list. 
Then the circuit is connected to the input of the corresponding AND gate. 
