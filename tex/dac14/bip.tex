...
A trace $t$ of length $l$ of a system $(B,\mathit{Init}, v)$ is the sequence of alternating states/configurations $q^1\cdot q^2 \cdots  q^l$ such that: $q^1$ = $(\mathit{Init}, v)$, and, $\forall i\in [2,l]: q^i\in Q \wedge \exists a_i\in A: q^i \goesto[a_i]_\pi q^{i+1}$, where $a_i$ is an interaction enabled on $q_i$. We denote by $t[i]$ the $i^{\text{th}}$ element in the trace, i.e., state $q_i$.
\end{definition}
