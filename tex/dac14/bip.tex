...
\todo{component(p), transition(p), interaction(p)}
...
Function
$transitions$ returns the transitions labeled with port $p$. Formally $transitions(p) = \set{\tau \in component(p)~|~\tau \in B.T \land \tau.port = p}$ 
