...
Consider the initial state: $t[0]$ and $t'[0]$. \cci{generateInitList()} sets \cci{cycle} to $\true$ and assigns each location to \textit{Init} and each variable to its initial value. Thus, $t[0]$ is equal $t'[0]$. 
\item
Let $t[0..k]$ be the prefix of $t$ of length $k+1$ where $k\ge 0$, using the induction hypothesis, there exists at least one trace 
  ...
  Otherwise, let $a_j$ be \redbox{an} interaction that is enabled according to state $t[k+1]$, 
