...
\begin{enumerate}
\item Consider the initial state: $t[0]$ and $t'[0]$. \cci{generateInitList()} sets \cci{cycle} to zero and assigns each location to \textit{Init} and each variable to its initial value. Thus, $t[0]$ is equal $t'[0]$. 
\item Let $t[0..k]$ be the prefix of $t$ of length $k+1$ where $k\ge 0$, using the induction hypothesis, there exists at least one trace 
  ...
  The primary input \cci{selector} wire variable is a nondeterministic variable and can assume all indices on interactions including the value 
...
\item Let $t'[0..k]$ be the prefix of $t'$ of length $k+1$ where $k\ge 0$, using the induction hypothesis, there exists a trace 
  ...
  Similarly to above, \cci{generateNextList()} guarantees that the state of the locations and the data variables are equal in the next states $t[k+1]$ and $t'[k+1]$ of $S$ and $P$, respectively. Furthermore, the two states are equivalent if no interaction was enabled in $t'[k+1]$. 
