...
\begin{enumerate}
\item Function \cci{generateDeclarationList()} fills \cci{decl-list} as follows. 
...
\end{lstlisting}
%
\item Function \cci{generateWireDefList()} fills \cci{wiredef-list} with functional macro definitions as follows.
...
\item Function \cci{generateInitList()} fills \cci{init-list} with initial value definitions taken from $\mathit{Init}$ for location variables ($B_i.\ell)$ and $v$ for component variables ($B_i.x_j)$.
...
\item Function \cci{generateNextList()} fills \cci{next-list} with the next state value definitions of register variables.
...
In the interaction mode (when $\mathit{cycle}$ is equal to zero), the function considers each assignment statement $\sigma$ from the action of interaction $a_j$.
...
Consider $T'$ the projection of $Tr_p$ constrained by omitting the states where \cci{cycle} is equal to zero. 
...
Consider the initial state: $t[0]$ and $t'[0]$. \cci{generateInitList()} sets \cci{cycle} to zero and assigns each location to \textit{Init} and each variable to its initial value. Thus, $t[0]$ is equal $t'[0]$. 
\item
Let $t[0..k]$ be the prefix of $t$ of length $k+1$ where $k\ge 0$, using the induction hypothesis, there exists at least one trace 
  ...
  and data variables will be the same at $t[k+1]$ and $t[k+1]$ as enforced by \cci{generateNextList()}. 
  ...
  The primary input \cci{selector} wire variable is a nondeterministic variable and can assume all indexes on interactions including the value 
  $j$, thus setting the enabled interaction $a_j$ in $t'[k+1]$. 
  Therefore, there exists a setting for \cci{selector} at step $k+1$ such that $P$ executes $a_j$. 
...
Let $t'[0..k]$ be the prefix of $t'$ of length $k+1$ where $k\ge 0$, using the induction hypothesis, there exists a trace 
  ...
  As for the previous proof, \cci{generateNextList()} guarantees that the state of the locations and the data variables are equal in the next states $t[k+1]$ and $t'[k+1]$ of $S$ and $P$, respectively. Furthermore, the two states are equivalent if no interaction was enabled in $t'[k+1]$. 
