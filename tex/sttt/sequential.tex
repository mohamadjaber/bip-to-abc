...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
The ABC synthesis and model checker reason about the And-Inverted-Graph representation of a sequential circuit~\cite{brayton2010abc}. 
%
...
trace $t$ is denoted by $t(v,i)$ (see \figref{fig:gate-value}).
\end{definition}
%
\begin{figure*}
\[
t(v,i)=
   ...
   \end{cases} \newline
\]
\caption{Value of gate $v$ at time $i$ in trace $t$}
\label{fig:gate-value}
\end{figure*}
%
Given a sequence of input valuations
...
%
We consider say that a variable declared as wires in \cci{decl-list}
...
\end{theorem}
%
The proof can be done by structural induction on $A$ and $P$. 
