...
wire boolean inter_timer_tmp;

int inter_done_prio;
wire boolean inter_done_tmp;
...
while(true) {
  @do_together {
    ...
    // port enablement
    porte_timer_done = (state_timer == s1) && (var_t == var_n) && (!portt_timer_done);  // ADD CONSTRAINTS FOR OTHER PORTS TRANSITIONS FOR PARANOID
    porte_timer_timer = (state_timer == s1) && (var_t < var_n) && (!portt_timer_timer); // ADD CONSTRAINTS FOR OTHER PORTS TRANSITIONS FOR PARANOID
    ...
    inter_timer_tmp = true  // guard of the interaction
      && porte_timer_timer; // port

    inter_done_tmp = true // guard of the interaction
    ...
    if(inter_done_tmp) {
      ...
      inter_done = ((!inter_timer_tmp) || (inter_timer_prio > inter_done_prio));// || (inter_timer_prio == inter_done_prio && inter_done_selected); // inter_done_selected comes for a nearest index encoder
    ...
    if(inter_timer_tmp) {
      inter_timer = ((!inter_done_tmp) || (inter_done_prio > inter_timer_prio));
      // || (inter_done_prio == inter_timer_prio && inter_done_selected); // inter_done_selected comes for a nearest index encoder
    ...
    porti_traffic_done = inter_done ; //if part of more than one interaction, then it is a disjunction
    porti_timer_done = inter_done;
    porti_timer_timer = inter_timer;
  }
}
